...
function GameHudlImport({ workspaceId, gameId }: { readonly workspaceId: string; readonly gameId: string }): ReactNode {
  ...
  return <Page>
    ...
    {step === 'mapping' && csv && <Panel className="grid gap-2 p-6">
      <h1 className="text-2xl font-semibold">Map CSV columns</h1>
      <p className="text-sm text-muted-foreground">{csv.rows.length} rows and {csv.headers.length} columns are ready to map.</p>
    </Panel>}
    {step === 'preview' && <Panel className="p-6">Preview</Panel>}
  </Page>
}
